...
@php
    ...
    if(session('aksi')== 'edit'){
        $id = $jenis->id;
        $bidang_id = $jenis->bidang_id;
        $namajenis = $jenis->nama;
        $parent =  $jenis->parent;
    }
@endphp
<form action="{{ route('backend.jenis.post') }}" method="post">
    {{ csrf_field()}}
    <div class="card">
		<div class="card-header header-elements-inline">
			<h5 class="card-title">Tambah / Ubah Jenis</h5>
		...
		<div class="card-body">
            ...
            <div class="form-group">
                ...
                <select name="bidang" class="form-control">
                    ...
                    <option value="{{ $v->id }}" @if($bidang_id == $v->bidang_id) selected @endif>{{$v->nama}}</option>
                    @endforeach
                </select>
            </div>
            <div class="form-group">
            ...
            <div class="form-group">
                <label>Nama:</label>
                <input type="text" class="form-control" name="namajenis" value="{{$namajenis}}" placeholder="Nama Bidang">
            </div>	
...
@endsection
@section('title')
<h4><span class="font-weight-semibold">Master</span> - Jenis</h4>
@endsection
@section('breadcrumb')
...
<span class="breadcrumb-item active">Jenis</span>
@endsection
@section('script-end')
@parent

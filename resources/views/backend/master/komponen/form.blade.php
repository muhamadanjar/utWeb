...
@php
    ...
    if(session('aksi')== 'edit'){
        $id = $jenis->id;
        $jenis_id = $komponen->jenis_id;
        $namakomponen = $komponen->nama;
        $parent =  $komponen->parent;
    }
@endphp
<form action="{{ route('backend.komponen.post') }}" method="post">
    {{ csrf_field()}}
    <div class="card">
		<div class="card-header header-elements-inline">
			<h5 class="card-title">Tambah / Ubah Komponen</h5>
		...
		<div class="card-body">
            ...
            <div class="form-group">
                ...
                <select name="jenis" class="form-control">
                    ...
                    <option value="{{ $v->id }}" @if($jenis_id == $v->jenis_id) selected @endif>{{$v->nama}}</option>
                    @endforeach
                </select>
            </div>
            <div class="form-group">
            ...
            <div class="form-group">
                <label>Nama:</label>
                <input type="text" class="form-control" name="namakomponen" value="{{$namakomponen}}" placeholder="Nama Komponen">
            </div>	
